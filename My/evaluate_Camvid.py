...
from models import networks_camvid
...
class Evaluator(object):
    def __init__(self, cfg):
        ...
        test_dataset = CamVidDataset(txt_dir=cfg["train"]["test_dir"],
                                     height=704,
                                     width=704,
                                     classes=cfg["model"]["classes"],
                                     augmentation=get_validation_augmentation(704, 704))

        # 初始化验证集的图片和标签，或是初始化test集
        self.image_paths = test_dataset.img_filename_list
        self.mask_paths = test_dataset.label_filename_list

        # 模型初始化
        self.model = networks_camvid.Mymodel().to(self.device)
        ...
        # 评价指标的初始化
        self.metric = SegmentationMetric(11)
    ...
    def eval(self):
        ...
        for i in range(len(self.image_paths)):
            image = Image.open("./datasets/CamVid/"+self.image_paths[i]).convert('RGB')  # image shape: (W,H,3)
            mask = Image.open("./datasets/CamVid/"+self.mask_paths[i])  # mask shape: (W,H)

            image = _img_transform(image)  # image shape: (3,H,W) [0,1]
            mask = torch.LongTensor(np.array(mask).astype('int32'))  # mask shape: (H,w)
            ...
            mask = get_color_pallete(mask, "camvid")
            ...
            pred = get_color_pallete(pred, "camvid")
...
if __name__ == '__main__':
    # 设置参数表
    config_path = "./configs/ReHalf_U2Net_Camvid.yaml"
