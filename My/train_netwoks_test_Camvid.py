...
from tools.losses import OhemCrossEntropy
...
class Trainer(object):
    def __init__(self, cfg):
        ...
        train_dataset = CamVidDataset(txt_dir=cfg["train"]["train_dir"],
                                      height=704,
                                      width=704,
                                      classes=cfg["model"]["classes"],
                                      augmentation=get_training_augmentation(704, 704))
        val_dataset = CamVidDataset(txt_dir=cfg["train"]["val_dir"],
                                    height=704,
                                    width=704,
                                    classes=cfg["model"]["classes"],
        ...
        # model初始化
        self.net = networks_camvid.Mymodel().to(self.device)
        ...
        self.criteria_pre = OhemCrossEntropy().to(self.device)
        # 评价指标初始化
    ...
    def train(self):
        ...
        for _ in range(self.epochs):
            ...
            for i, (images, targets) in enumerate(self.train_dataloader):
                ...
                loss = self.criteria_pre(outputs, targets.long())
